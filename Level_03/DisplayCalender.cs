/*8. Create a program to display a calendar for a given month and year. The program
...
using System;
internal static class DisplayCalender
{
    public static void Main()
    {
        Console.Write("Enter month (1-12): ");
        int month = int.Parse(Console.ReadLine());
        Console.Write("Enter year (example, 2024): ");
    ...
    public static int GetFirstDayOfMonth(int month, int year)
    {
        ...
        int k = year % 100;
        int j = year / 100;
        int d0 = (1 + (13 * (month + 1)) / 5 + k + (k / 4) + (j / 4) + (5 * j)) % 7;
        return (d0 + 5) % 7; // Adjusting to make Sunday = 0
    }
}
